...
Ya sabemos el problema: lo que se carga siempre ocupa contexto, y si crece, rinde peor. La solución es sencilla de decir y tiene truco al aplicarla: sacar el detalle del fichero que se carga siempre y llevarlo a otro que solo se lea cuando haga falta.
...
Las **skills**. Cargan solo cuando las invocas o cuando el asistente ve que hacen falta para tu petición. Son el sitio natural de un procedimiento largo de varios pasos que no tiene que estar en contexto todo el rato.
...
## Núcleo caliente, archivo frío

La forma que sale de todo esto tiene dos capas. Un **núcleo caliente**, corto y estable, que se carga siempre. Y un **archivo frío**, con el detalle, que se trae bajo demanda. No es un invento nuestro: repos de referencia que trabajan este problema llegan a la misma estructura de dos capas, con un fichero raíz de entrada por debajo de 50 o 60 líneas. Cuando dos personas resuelven un problema por separado y llegan al mismo sitio, suele ser que el sitio es el correcto.
